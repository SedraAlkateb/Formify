...
Widget surveyListWidget(MainSurveyModel survey) {
  return Row(
    children: [
      // Left border container with dynamic height
      Container(
        width: 4,  // Thin left border
        color: survey.color.contains("0x")
            ? Color(int.parse(survey.color))
            : Colors.blue,  // Use dynamic color
        // Removed fixed height, it will fill based on its parent (ListView)
      ),
      Expanded(
        child: Card(
